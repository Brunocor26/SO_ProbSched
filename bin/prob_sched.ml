...
let usage_msg = "Usage: " ^ Sys.argv.(0) ^ " --algo <name> --file <path> [--quantum <int>]"
let speclist = [
...
let run_and_output () =
  try
    ...
    let is_realtime = match String.lowercase_ascii algo with "rm" | "edf" -> true | _ -> false in
    ...
    let simulation_result =
      try
        match algo with
        | "fcfs" -> Some (Scheduler.fcfs processos_iniciais)
        | "sjf" -> Some (Scheduler.sjf processos_iniciais)
        | "priority_np" -> Some (Scheduler.priority_non_preemptive processos_iniciais)
        | "priority_preemp" -> Some (Scheduler.priority_preemptive processos_iniciais)
        | "rr" -> (match quantum with
                  | Some q -> Some (Scheduler.round_robin ~quantum:q processos_iniciais)
                  | None -> failwith "Internal error: Quantum missing for RR")
        (*| "rm" -> Some (Scheduler.rate_monotonic processos_iniciais)
        | "edf"-> Some (Scheduler.edf processos_iniciais)*)
        | _ -> failwith ("Algorithm '" ^ algo ^ "' not recognized or implemented.")
      with ex -> failwith ("Error during simulation for algorithm '" ^ algo ^ "': " ^ Printexc.to_string ex)
    in

    match simulation_result with
    | None -> failwith "Simulation failed to produce results."
    | Some (tempo_final, log_eventos) ->
        let stats = Statistics.calculate_statistics processos_iniciais tempo_final in
